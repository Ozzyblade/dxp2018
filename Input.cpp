...
void MouseAndKeys::MousePosToWorldRay(const Matrix& view, const Matrix& proj, const Vector3& camPos, Ray& ray)
{
	...
	float x = (2 * mMouseScreen.x / w) - 1;			// X value between -1 and 1.
	float y = ((2 * mMouseScreen.y / h) - 1) * -1;	// Y value between -1 and 1 and INVERT IT
	x = x / proj.m[0][0];		// Undo the scaling effect within a Projection Matrix (Don't know their typical values off hand).
	y = y / proj.m[1][1];		// Projection will be distorting values (E.G. a fisheye lens projection?).
